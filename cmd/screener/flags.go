...
func (c *CLI) usage() {
	...
	fmt.Fprintf(w, "\t%s,  %s\t%s\t(Default: %v)\n", "-wn", "--wait-for-network-idle", "Wait for all network activity to complete before capturing", screener.DefaultOptions().WaitForNetworkIdle)
...
func (c *CLI) parseFlags() {
	...
	flag.BoolVar(&c.Options.WaitForNetworkIdle, "wait-for-network-idle", screener.DefaultOptions().WaitForNetworkIdle, "")
	flag.BoolVar(&c.Options.WaitForNetworkIdle, "wn", screener.DefaultOptions().WaitForNetworkIdle, "")
