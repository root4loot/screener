...
func (c *CLI) usage() {
	...
	fmt.Fprintf(w, "\t%s,  %s\t%s\t(Default: %v)\n", "-wb", "--wait-for-body", "wait for page body to load", screener.DefaultOptions().WaitForPageBody)
...
func (c *CLI) parseFlags() {
	...
	flag.BoolVar(&c.Options.WaitForPageBody, "wait-for-body", screener.DefaultOptions().WaitForPageBody, "")
	flag.BoolVar(&c.Options.WaitForPageBody, "wb", screener.DefaultOptions().WaitForPageBody, "")
